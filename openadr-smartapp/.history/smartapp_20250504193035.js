...
const CLIENT_SECRET = process.env.SMARTAPP_CLIENT_SECRET || '8d99d940-a884-45bb-b818-69a927cd1eee'
...
module.exports = new SmartApp()
  ...
  .page('mainPage', (context, page) => {
    ...
    page.section('tempSensors', section => {
      section.deviceSetting('tempSensors')
        .capabilities(['temperatureMeasurement'])
        .permissions('r')
        .multiple(true)
        .required(false)
        .label('Optional Temperature Sensors')
        .description('Include sensors if you want temperature telemetry')
    })
  ...
  .subscribedEventHandler('tempHandler', (context, event) => {
    console.log(`[TEMP] device=${event.deviceId} value=${event.value}${event.unit ?? ''}`)
  })
...
async function setupSubscriptions(context) {
  ...
  if (context.config.tempSensors) {
    await context.api.subscriptions.subscribeToDevices(
      context.config.tempSensors,
      'temperatureMeasurement',
      'temperature',
      'tempHandler'
    )
    console.log(`[DEBUG] Subscribed to temperature for ${context.config.tempSensors.map(d => d.deviceId)}`)
  }
  ...
  cache.set(appId, {
    ...
    tempSensors: (context.config.tempSensors || []).map(d => d.deviceId)
  })
